...
// Read an env var override safely. Returns null in any of these cases:
//   - unset
//   - literal empty string (idiomatic JS would fall through to default; this
//     just makes the behaviour explicit so a future `||` → `??` refactor
//     doesn't change semantics)
//   - whitespace-only or contains control chars (e.g. `$(cat file)` in CI
//     leaves trailing newlines that would otherwise propagate into a path
//     and cause silent misconfiguration)
// Trims surrounding whitespace before returning — accepts `' /tmp/foo '` as
// `/tmp/foo`, but rejects `'\n/tmp/foo'` because of the control char.
...
// The bridge MCP server's diagnostic log (separate from the daemon's because
// the two run in different processes). Same reasoning: previously
// /tmp/copilot-bridge.log was world-readable and recorded request payloads.
export function bridgeLogPath() {
  return join(runtimeDirPath(), 'bridge.log');
}

// OpenTelemetry traces file consumed by Copilot CLI itself (we hand it the
// path via COPILOT_OTEL_FILE_EXPORTER_PATH). Copilot's traces include prompt
// content and tool I/O, so this MUST live inside the 0o700 dir to prevent
// other local users from reading it.
export function otelTracesPath() {
  return join(runtimeDirPath(), 'otel-traces.jsonl');
}
