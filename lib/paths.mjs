...
function computeNamespace() {
  ...
  // Fall back to username on platforms without getuid (Windows). Restrict
  // to a portable charset so the value is safe to interpolate into paths.
  try {
...
function _runtimeDirBase() {
  if (process.env.COPILOT_RUNTIME_BASE) return process.env.COPILOT_RUNTIME_BASE;
...
export function ensureRuntimeDir() {
  ...
  // Windows: no POSIX uid/mode. Just create the dir; the security model
  // here is NTFS-ACL-based and out of scope for this function.
  if (typeof process.getuid !== 'function') {
...
export function socketPath() {
  return process.env.COPILOT_SOCKET_PATH
    || join(runtimeDirPath(), 'daemon.sock');
}

export function queuePath() {
  return process.env.COPILOT_QUEUE_PATH
