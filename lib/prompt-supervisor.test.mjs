...
describe('F1g — no tool-calls-without-message cap', () => {
  it('long fan-out of distinct, succeeding tool_calls never trips on count alone', () => {
    // Research-template fan-out (parallel web fetches via /fleet) routinely
    // produces hundreds of distinct tool_calls before the outer session
    // synthesizes a message. Genuine stuck cases are caught by poll-level
    // silenceTripMs; loops by hash/view detection; errors by failure streaks.
    const sup = new Supervisor();
    const state = freshState();
    for (let i = 0; i < 500; i++) {
      const r = sup.observe({ type: 'tool_call', toolCallId: 't_' + i, name: `distinct_${i}`, kind: 'read', input: { path: `/f_${i}` }, ts: T0 + i }, state);
      assert.equal(r.action, 'ok', `event ${i + 1} should not trip`);
    }
  });
});
